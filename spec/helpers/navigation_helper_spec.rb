...
RSpec.describe NavigationHelper, type: :helper do 
  ...
  context 'signed in user' do 
    before(:each) do 
      allow_any_instance_of(Devise::Controllers::Helpers).to receive(:user_signed_in?)
        .and_return(true)
    end

    context '#collapsible_links_partial_path' do 
      it 'returns signed_in_links_partials path' do 
  ...
  context 'non-signed in user' do 
    ...
    context '#collapsible_links_partial_path' do 
      it "returns non_signed_in_links_partial" do 
        expect(helper.collapsible_links_partial_path).to
